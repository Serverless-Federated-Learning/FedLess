...
from fedless.aggregator.aggregation import FedAvgAggregator
...
def run(
    ...
    global_params = SerializedParameters(
        blob=weight_bytes,
        serializer=WeightsSerializerConfig(type="npz", params=NpzWeightsSerializerConfig()),
    )
    ...
    while test_accuracy < 0.95 and epoch < epochs:
        ...
        new_parameters_bytes = NpzWeightsSerializer(compressed=False).serialize(new_parameters)
        ...
        round_results.append(
            {
                ...
                "privacy_guarantees": [
                    result.privacy_guarantees.json() for result in results if result.privacy_guarantees
                ],
                **mia_results,
            }
        )
