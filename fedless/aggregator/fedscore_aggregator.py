...
class FedScoreAggregator(StallAwareParameterAggregator):
    def _aggregate(
        self,
        ...
        logger.info(f"[ASYNC] - with tolerance {self.tolerance}")
        result_age = [stat["round_id"] - self.current_round for stat in client_stats]
        age_dist = {i: result_age.count(i) for i in set(result_age)}
        nr_current_result = age_dist.get(0, 0)
        logger.info(
            f"*** current: {nr_current_result}, stale: {len(client_stats)-nr_current_result} -> age distribution={age_dist}"
        )

        # Default tolerance = 0 => default score = 1 for all
        client_staleness_scores = self._asyn_staleness_score(client_stats)
        weighted_parameters = [
        ...
        # if stale result exists -> Compensate with global parameters
        total_client_weight = sum(
            np.multiply(client_staleness_scores, client_cardinalities)
        )

        # get average parameter
        avg_parameters: List[np.ndarray] = [
            reduce(np.add, layer_updates) / total_client_weight
...
class StreamFedScoreAggregator(FedScoreAggregator):
