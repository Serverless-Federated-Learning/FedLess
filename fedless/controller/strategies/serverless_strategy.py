...
class ServerlessFlStrategy(FLStrategy, ABC):
    def __init__(
        ...
        self.mock = mock
        self.mock_aggregator = mock_aggregator
    ...
    def save_client_scores(self):
        client_score_dao = ClientScoreDao(db=self.mongodb_config)
        client_score_list: List[ClientScore] = client_score_dao._collection.find(
            {"session_id": self.session}, {"_id": False}
        )
        keys = ClientScore.__fields__.keys()
        with open(self.save_dir / f"client_scores_{self.session}.csv", "w") as file:
    ...
    async def fit_round(
        ...
        logger.info(
            f"{len(succs)} clients returned training results, {len(errors)} failures... {errors}"
        )
        ...
        logger.info(
            f"Received results from {len(succs)}/{len(clients)} client functions ... ({clients[0].hyperparams.strategy} - {clients[0].data.type})"
        )

        t_clients_end = time.time()

        self.save_client_scores()
        preps_dict = {
            "succs": len(succs),
            "failed": len(errors),
        ...
        # only store client history for FedlesScan
        if isinstance(self.selectionStrategy, DBScanClientSelection):
            ...
            for failed_client_id in all_clients:
                ...
                if failed_client.client_backoff <= 0:
                    ...
                    logger.info(
                        f"using exponential backoff for client {failed_client_id}"
                    )
        ...
        agg_res: AggregatorFunctionResult = (
            ...
            if self.mock or self.mock_aggregator
            else self.call_aggregator(round)
        )

        if not self.is_synchronous:
            self.nr_last_complete_clients = agg_res.num_clients + len(errors)
        ...
        if agg_res.global_test_results:
            ...
            if hasattr(self, "evaluate_clients"):
                ...
                if not self.is_synchronous:
                    # prevent busy client for being invoked (only relevent in asyn mode)
                    busy_client_ids = (
                        self.invocation_history_dao.get_busy_inv_client_ids(
                            self.session
                        )
                    )
                    available_clients = list(
                        filter(
                            lambda client: not client.client_id in busy_client_ids,
                            self.clients,
                        )
                    )
                else:
        ...
        metrics_misc.update(
            {
                ...
                "num_aggregated_client": agg_res.num_clients,
